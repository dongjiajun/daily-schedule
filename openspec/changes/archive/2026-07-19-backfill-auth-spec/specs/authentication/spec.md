...
<!-- 参考: specs/openapi.yaml + docs/api/overview.md -->
...
|----------|--------|----------|------|
| 生成 access token | JwtUtilTest | `accessToken_roundTrip` | ✅ |
| 生成 refresh token | JwtUtilTest | `refreshToken_roundTrip` | ✅ |
| v1.1 兼容入口 | JwtUtilTest | `legacyEntryEquivalent` | ✅ |
| 正确类型匹配 | JwtUtilTest | `accessToken_roundTrip` / `refreshToken_roundTrip` | ✅ |
| 类型不匹配返回 null | JwtUtilTest | `typeMismatch_returnsNull` | ✅ |
| 无类型校验模式 | JwtUtilTest | `noTypeCheck_returnsClaimsRegardless` | ✅ |
| 篡改 token 返回 null | JwtUtilTest | `tampered_returnsNull` | ✅ |
| null/blank token 返回 null | JwtUtilTest | `blank_returnsNull` | ✅ |
| Bearer header 认证 | JwtAuthFilterTest | `bearerHeader_setsAuthentication` | ✅ |
| Cookie 回退认证 | JwtAuthFilterTest | `cookieFallback_setsAuthentication` | ✅ |
| Bearer 优先于 Cookie | JwtAuthFilterTest | `bearerWinsOverCookie` | ✅ |
| 无效 token 不阻断请求 | JwtAuthFilterTest | `invalidToken_noAuthentication` | ✅ |
| 非 Bearer header 回退 Cookie | JwtAuthFilterTest | `nonBearerHeader_fallsBackToCookie` | ✅ |
| 有效注册创建用户和默认分类 | AuthApplicationServiceTest | `register_validInput_createsUserAndCategories` | ✅ |
| displayName 省略使用 username | AuthApplicationServiceTest | `register_nullDisplayName_usesUsername` | ✅ |
| 重复用户名返回 409 | AuthApplicationServiceTest | `register_duplicateUsername_throws` | ✅ |
| 重复邮箱返回 409 | AuthApplicationServiceTest | `register_duplicateEmail_throws` | ✅ |
| 非法输入返回 400 | AuthApplicationServiceTest | `register_shortPassword_rejected` / `register_invalidEmail_rejected` / `register_invalidUsername_rejected` | ✅ |
| 用户名/邮箱登录成功 | AuthApplicationServiceTest | `login_validCredentials_returnsTokens` | ✅ |
| 密码错误返回 401 | AuthApplicationServiceTest | `login_wrongPassword_throws` | ✅ |
| 用户不存在返回 401 | AuthApplicationServiceTest | `login_userNotFound_throws` | ✅ |
| 已禁用用户返回 401 | AuthApplicationServiceTest | `login_disabledUser_throws` | ✅ |
| 有效 refresh 换发新 token | AuthApplicationServiceTest | `refresh_validToken_returnsNewTokens` | ✅ |
| access 当 refresh 被拒绝 | AuthApplicationServiceTest | `refresh_wrongTokenType_throws` | ✅ |
| 已禁用用户续签被拒 | AuthApplicationServiceTest | `refresh_disabledUser_throws` | ✅ |
| 并发请求共享单次 refresh | — | — | ⚠️ 前端逻辑，无后端覆盖 |
| 网络错误不强制登出 | — | — | ⚠️ 前端逻辑，无后端覆盖 |
| HTTP 401 强制登出 | — | — | ⚠️ 前端逻辑，无后端覆盖 |
| 正常登出清除本地状态 | — | — | ⚠️ 前端逻辑，无后端覆盖 |
