...
|----------|--------|----------|------|
| 春节自动切换 | useTheme.test.ts | shouldAutoSwitchForSpringFestival | ➕ |
| 手动模式不覆盖 | useTheme.test.ts | shouldNotOverrideManualTheme | ➕ |
| 无节日使用手动 | useTheme.test.ts | shouldFallbackToManualOnNoHoliday | ➕ |
| 节日结束恢复 | useTheme.test.ts | shouldRestoreAfterHoliday | ➕ |
| CSS 变量映射 | useTheme.test.ts | shouldMapCssVariables | ➕ |
| locale 传递 | useTheme.test.ts | shouldPassLocaleToEngine | ➕ |
