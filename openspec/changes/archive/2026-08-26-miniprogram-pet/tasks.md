...
- [x] 9.4 Smoke test — 微信开发者工具手工验证（需先 `cd apps/miniprogram && pnpm run build` 导入 dist/，注意清缓存→清除全部缓存→编译）：
  - [x] 无宠物 → 宠物页展示创建引导 → 选物种/命名 → 创建成功进入展示态
  - [x] 宠物游走动画：wandering 目标驱动 + 平滑移动 + 视口内不越界
  - [x] 喂食/玩耍 → 反馈动画 + 数值变化（+情绪/饥饿/经验）+ 互动中按钮置忙
  - [x] 互动失败/网络断开 → 错误提示，状态保持原值
  - [x] 401 静默重登：token 失效后操作自动无感重登并重拉
  - [x] TabBar 5 入口：首页/日历/任务/宠物/我的 可切换，宠物页入口正常
