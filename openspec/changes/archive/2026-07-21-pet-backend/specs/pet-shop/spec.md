...
#### Scenario: 商店为空（无种子数据）
- **WHEN** 数据库 `pet_accessories` 表为空（理论上不应发生，Flyway 已插入种子数据）
- **THEN** 服务器返回 200，空数组 `[]`
...
|----------|--------|----------|------|
| 获取物品列表 | PetControllerTest | shouldListShopItems | ➕ |
| 成功购买 | PetControllerTest | shouldPurchaseItem | ➕ |
| 专注币不足 | PetControllerTest | shouldRejectPurchaseNoCoins | ➕ |
| 物品不存在 | PetControllerTest | shouldRejectInvalidItem | ➕ |
| 批量购买 | PetApplicationServiceTest | shouldHandleBatchPurchase | ➕ |
| 无宠物 | PetControllerTest | shouldReturn404WhenNoPet | ➕ |
