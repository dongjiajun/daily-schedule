...
### Requirement: 喂食宠物
用户 SHALL 能使用专注币购买食物并喂给宠物。喂食增加饱腹度和心情，获得少量经验。消耗当前拥有的食物库存（v1 简化：直接在 purchase 时消耗，不建 inventory 表）。

#### Scenario: 成功喂食
- **WHEN** 拥有宠物且专注币 ≥ 10，发送 `POST /api/v1/pets/me/interact`，body: `{ type: "FEED", quantity: 1 }`
- **THEN** 服务器返回 200，响应体为 InteractionResult：`hungerChange = +20`, `moodChange = +5`, `experienceGain = +3`, `coinChange = -10`（默认用"小鱼干"的价格和效果），`newHunger`, `newMood`, `newExp`, `newCoins` 为操作后的当前值
...
|----------|--------|----------|------|
| 成功喂食 | PetControllerTest | shouldFeedPet | ➕ |
| 专注币不足 | PetControllerTest | shouldRejectFeedWhenNoCoins | ➕ |
| 喂食上限 | PetDomainServiceTest | shouldCapHungerAt100 | ➕ |
| 成功玩耍 | PetControllerTest | shouldPlayWithPet | ➕ |
| 玩耍饱腹下限 | PetDomainServiceTest | shouldCapHungerAt0 | ➕ |
| 互动记录 | PetApplicationServiceTest | shouldRecordInteraction | ➕ |
| 跨用户隔离 | PetApplicationServiceTest | shouldIsolateInteraction | ➕ |
