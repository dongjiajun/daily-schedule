...
- [x] 9.3 Smoke test — 启动前后端，浏览器验证关键路径：
  - [x] 登录 → 点击"任务看板"导航 → 看板三列显示
  - [x] 在 TODO 列点击"+ 新建" → 输入标题回车 → 任务卡片出现在 TODO 列
  - [x] 拖拽任务卡片从 TODO 到 IN_PROGRESS → 卡片移动成功
  - [x] 拖拽任务卡片到 DONE → 宠物播放开心动画 + 气泡消息
  - [x] 切换到列表视图 → 任务按行显示 → 状态下拉框切换成功
  - [x] 创建带截止日期和标签的任务 → 编辑 → 删除
