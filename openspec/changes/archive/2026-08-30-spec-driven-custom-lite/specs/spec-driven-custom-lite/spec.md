...
|----------|--------|----------|------|
| 小变更走 lite 链 | openspec CLI（schema 定义） | 本变更冒烟：TEMP 根 `--schema` 新建 | ➕ |
| 默认 schema 不受影响 | openspec/config.yaml（schema: spec-driven-custom） | 不带 --schema 新建回验 | ➕ |
| 小行为变更用 lite | 规则（适用范围 Requirement） | 本变更冒烟 | ➕ |
| custom 模板变更后 lite 同步 | openspec schema validate | — | ➕ |
| lite 冒烟验证 | openspec schema validate + status | — | ➕ |
