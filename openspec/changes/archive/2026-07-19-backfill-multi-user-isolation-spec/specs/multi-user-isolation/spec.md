...
<!-- 参考: docs/architecture.md + docs/database/schema.md -->
...
|----------|--------|----------|------|
| 已认证获取 userId | CurrentUserServiceTest | `principalLong_returnsId` | ✅ |
| 未认证抛异常 | CurrentUserServiceTest | `noAuth_throws` | ✅ |
| principal 类型错误 | CurrentUserServiceTest | `wrongPrincipalType_throws` | ✅ |
| Bearer header 注入 principal | JwtAuthFilterTest | `bearerHeader_setsAuthentication` | ✅ |
| 访问他人事件返回 404 | EventApplicationServiceTest | (via `getById` ownership check) | ✅ |
| 事件不存在返回 404 | EventApplicationServiceTest | `delete_notFound_shouldThrow` | ✅ |
| SQL 级 user_id 过滤 | EventRepositoryImplTest | (via MyBatis queries with userId param) | ✅ |
| 分类重名同用户独立 | — | — | ⚠️ 无显式跨用户唯一性测试 |
| findUpcoming 不过滤 userId | ReminderSchedulerTest | (via mock setup returning multi-user events) | ✅ |
| 提醒仅推送给所属用户 | SseEmitterManagerTest | `sendToUser_invokesCorrectUserEmitters` | ✅ |
