...
`EventApplicationService.update()` SHALL 先通过 `getById` 做归属校验，然后调用 `existing.update(data)` 部分更新（仅覆盖非 null 字段），再调用 `existing.isValid()` 校验。SHALL NOT 重新执行冲突检测。
...
|----------|--------|----------|------|
| 新建事件默认 PLANNED | EventDomainServiceTest | (implicit via fromString) | ✅ |
| PLANNED 参与冲突检测 | EventApplicationServiceTest | `create_timeConflict_shouldThrow` | ✅ |
| COMPLETED 不参与冲突检测 | EventApplicationServiceTest | `create_overlapWithCompleted_shouldNotConflict` | ✅ |
| 时间段重叠报告冲突 | EventDomainServiceTest | `hasTimeConflict_whenOverlap_returnsTrue` | ✅ |
| 相邻不冲突 | EventDomainServiceTest | `hasTimeConflict_whenAdjacent_returnsFalse` | ✅ |
| 完全包含报告冲突 | EventDomainServiceTest | `hasTimeConflict_whenContaining_returnsTrue` | ✅ |
| 编辑自身排除 | EventDomainServiceTest | `hasTimeConflict_whenEditingSelf_returnsFalse` | ✅ |
| 无已有事件不冲突 | EventDomainServiceTest | `hasTimeConflict_whenEmpty_returnsFalse` | ✅ |
| 部分更新保留未修改字段 | EventDomainServiceTest | `update_onlyAppliesNonNullFields` | ✅ |
| 无效事件拒绝 | EventDomainServiceTest | `isValid_whenEndBeforeStart_returnsFalse` + `isValid_whenTitleEmpty_returnsFalse` | ✅ |
| CANCELLED 不触发提醒 | ReminderSchedulerTest | `noReminderMinutes_skips` (via SQL filter) | ✅ |
| PLANNED↔COMPLETED toggle | — | — | ⚠️ 前端逻辑，无后端覆盖 |
| 按状态过滤 | — | — | ⚠️ Controller 层，无独立单测断言 |
