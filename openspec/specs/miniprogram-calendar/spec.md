...
## Purpose
Phase 2 主链（M2.1-2.2）第三个能力：小程序第一个业务页面「日历月视图只读」——42 格月网格 + 当月事件展示 + 选中日事件列表，复用后端现有 `GET /api/v1/events`（Bearer 鉴权链路首次被业务消费），后端与 API 契约零变更。日期处理为字符串切片纯函数（iOS JSC 不解析无时区 ISO 串），401 走清态 + 静默重登兜底（refresh 预续签留待后续变更）。
