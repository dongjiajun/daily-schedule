...
## Purpose
Phase 2 主链（M2.1-2.2）第五个能力：小程序 TabBar「宠物」入口——宠物状态展示 + 创建引导 + 喂食/玩耍互动 + 游走动效，复用后端 `/pets/me` 三端点（GET/POST/POST interact，API 契约零变更）。404 = 无宠物业务态（创建引导）；游走复用 `@daily-schedule/shared/pet` roam engine（`computeNextTarget` wandering 模式 + 随机节奏），View 绝对定位 px + CSS transition，setTimeout 链驱动并双路清理；宠物形象为 emoji 🐱/🐕 + 颜色圈底（微信 image 不支持 svg）；坐标全程 px（引擎 px 语义）。
...
- **THEN** 展示宠物名称、物种中文标签、等级、经验数值、心情条、饥饿条与金币数值
...
| Scenario | 测试 | 状态 |
|----------|------|------|
| 响应校验（字段/枚举/非法值抛「宠物数据格式异常」）+ null 视为缺省 | pet.test.ts parsePetProfile 6 用例 + null 兼容用例 | ✅ |
| 状态换算（百分比 clamp/三段色/心情饥饿标签分段） | pet.test.ts 换算纯函数 4 用例 | ✅ |
| 404 映射 null / 401 透传 / 非 404 错误 message 上抛 / 校验失败 | pet.test.ts fetchMyPet 5 用例 | ✅ |
| 四函数请求路径与 body / 响应解析 / 错误透传（含 null 字段容忍） | pet.test.ts createPet 2 + interactWithPet 7 用例 | ✅ |
| 页面渲染（状态条/游走动画/互动反馈/创建表单 UI） | 无渲染级测试（小程序渲染测试基础设施待后续变更引入，见技术债 MP2）→ 9.4 smoke 手工覆盖：创建/游走/喂食/玩耍/401/五 tab 用户已验证 | ✅ |
