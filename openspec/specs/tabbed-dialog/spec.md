...
从日历的 `ManageDialog` 中提取通用的标签对话框模式，使任意模块都能以声明方式创建标签页管理界面。
...
- `activeTab?: string` — 受控模式的当前标签页
- `onActiveTabChange?: (tabId: string) => void` — 受控模式的切换回调
- `defaultTab?: string` — 非受控模式的默认标签页
