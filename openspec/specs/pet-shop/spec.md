...
### Requirement: 查看商店物品
用户 SHALL 能查看所有可购买的物品列表（FOOD 食物与 ACCESSORY 配饰两类），每个物品包含名称、类型、价格、效果（心情/饱腹/经验增益；配饰效果为 0）。
...
#### Scenario: 商店包含食物与配饰
- **WHEN** 种子数据就位，发送 `GET /api/v1/shop/items`
- **THEN** 返回列表同时包含 `type="FOOD"` 食物与 `type="ACCESSORY"` 节日配饰
...
### Requirement: 购买物品
用户 SHALL 能用自己的专注币购买商店物品。购买语义 SHALL 按类型分流（详见 pet-accessory-equip）：FOOD 为即时消费模型（购买后效果立即应用到宠物，无库存）；ACCESSORY 为购买即装备（写入 `current_accessory`，覆盖旧装备，quantity 固定 1）。专注币余额不足时拒绝。
...
#### Scenario: 购买配饰即装备
- **WHEN** 专注币充足，发送 `POST /api/v1/shop/purchase`，body: `{ itemId: 配饰id, quantity: 1 }`
- **THEN** 服务器返回 200：`success = true`，`equippedAccessoryId = 配饰id`，宠物数值不变、专注币扣减价格

#### Scenario: 配饰批量购买拒绝
- **WHEN** 购买 ACCESSORY 物品且 `quantity > 1`
- **THEN** 服务器返回 400，错误信息包含"配饰每次只能购买一件"
...
#### Scenario: 批量购买（食物）
- **WHEN** `quantity > 1`（如 quantity=3）购买 FOOD 物品，专注币足够
