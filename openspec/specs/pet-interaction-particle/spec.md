...
互动粒子爆发系统：短暂局部特效（爱心/星星/金币/火花），在喂食/玩耍等互动成功时播放，强化即时反馈与视觉趣味。
...
- **WHEN** 宠物触发 `happy`/`excited` 状态或用户玩耍
- **THEN** 发射 8-12 个 ⭐ 粒子，向四周散开
- **THEN** 粒子持续 2s

#### Scenario: Food particles on feed
- **WHEN** 用户喂食宠物
- **THEN** 发射 5-8 个食物粒子（🍖🍗🦴🥕），从宠物位置向上飘散
- **THEN** 粒子持续 1.5s

#### Scenario: Coin particles on purchase
- **WHEN** 用户购买成功（`POST /shop/purchase`）
- **THEN** 发射 3-5 个 🪙 粒子，从宠物位置向四周散开
- **THEN** 粒子持续 1s
