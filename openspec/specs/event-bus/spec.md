...
定义模块间唯一通信通道——类型安全的同步事件总线。SystemEvent 联合类型覆盖所有跨模块事件（日程/任务/习惯/专注/用户），EventBus 类提供同步的 on/emit/off/removeAll API。事件类型定义在 shared 包中，供 Web + 小程序共用；frontend 中创建单例实例。
