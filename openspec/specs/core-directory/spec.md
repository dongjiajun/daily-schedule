...
定义 `frontend/src/core/` 核心目录结构——将稳定基础设施代码（stores、lib 工具、shadcn/ui 组件、共享 hooks、主题样式）从 `src/` 顶层收敛到 `core/` 下，与 `modules/` 形成清晰的架构边界。业务专属代码（日历 hooks/store）保留在 `src/` 顶层，留待后续迁移到具体模块目录。
