...
async function loadConfigFile({ configFile, logger, name, userConfig }) {
	...
	return {
		...
		foundConfig: fileConfigs.some((config) => Object.keys(config).length)
	};
}
//#endregion
//#region src/log.ts
	...
	*/
	endAllEvents(events) {
		for (const event of events) {
			...
			if (event.events.length) this.endAllEvents(event.events);
		}
	}
	report(print = true) {
...
var Planner = class {
	...
	*/
	assignSymbolName(args) {
		...
		for (const scope of updateScopes) this.updateScope(symbol, scope);
	}
	/**
	...
	*/
	nameIsAvailable({ kind, language, name, scope }) {
		let current = scope;
		while (current) {
			const kinds = current.localNames.get(name);
			if (kinds) {
				for (const existingKind of kinds) if (!canDeclarationsShareIdentifier(language, kind, existingKind)) return false;
			}
...
var SymbolRegistry = class {
	...
	*registered() {
	...
	buildIndexKeySpace(meta, prefix = "") {
		...
		for (const [key, value] of Object.entries(meta)) {
			...
			if (value && typeof value === "object" && !Array.isArray(value)) entries.push(...this.buildIndexKeySpace(value, path));
			else entries.push([
				path,
				value,
	...
	*/
	cacheKeyFromKeySpace(indexKeySpace) {
		return indexKeySpace.map((indexEntry) => indexEntry[2]).sort().join("|");
	}
	...
	invalidateCache(indexKeySpace) {
		for (const indexEntry of indexKeySpace) {
			...
			for (const cacheKey of cacheKeys) {
				...
				if (deps) {
					for (const dep of deps) if (dep[2] !== serialized) this._dependencyToCache.get(dep[2])?.delete(cacheKey);
	...
	queryByKeySpace(indexKeySpace, cacheKey) {
		...
		for (const indexEntry of indexKeySpace) {
			const serialized = indexEntry[2];
			let cacheKeys;
			if (this._dependencyToCache.has(serialized)) cacheKeys = this._dependencyToCache.get(serialized);
			else {
		...
		for (let i = 1; i < sets.length; i++) {
			const set = sets[i];
			for (const symbolId of result) if (!set.has(symbolId)) result.delete(symbolId);
		}
	...
	*/
	insert(args) {
		...
		for (const location of locations) {
			...
			const fullPath = path.filter((s) => Boolean(s));
