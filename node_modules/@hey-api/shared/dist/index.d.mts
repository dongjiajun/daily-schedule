...
//#endregion
//#region src/config/input/types.d.ts
...
type UserWatch = boolean | number | Watch;
type Watch = {
  /**
...
//#endregion
//#region src/config/input/input.d.ts
declare function getInput(userConfig: {
...
interface IROperationObject extends SpecExtensions {
  body?: IRBodyObject;
  deprecated?: boolean;
  description?: string;
  id: string;
...
interface IRParameterObject extends Pick<JSONSchemaDraft2020_12.Document, 'deprecated' | 'description'>, SpecExtensions {
  /**
...
interface IRSchemaObject extends Pick<JSONSchemaDraft2020_12.Document, '$ref' | 'const' | 'default' | 'deprecated' | 'description' | 'exclusiveMaximum' | 'exclusiveMinimum' | 'maximum' | 'maxItems' | 'maxLength' | 'minimum' | 'minItems' | 'minLength' | 'pattern' | 'required' | 'title'>, Pick<OpenAPIV3_1.SchemaObject, 'example'>, SpecExtensions {
  /**
...
declare namespace IR {
  type BodyObject = IRBodyObject;
  type ComponentsObject = IRComponentsObject;
  type Model = IRModel;
  type OperationObject = IROperationObject;
  type ParameterObject = IRParameterObject;
  type ParametersObject = IRParametersObject;
  type PathItemObject = IRPathItemObject;
  type PathsObject = IRPathsObject;
  type ReferenceObject = OpenAPIV3_1.ReferenceObject;
  type RequestBodyObject = IRRequestBodyObject;
  type ResponseObject = IRResponseObject;
  type ResponsesObject = IRResponsesObject;
  type SchemaObject = IRSchemaObject;
  type SecurityObject = IRSecurityObject;
  type ServerObject = IRServerObject;
 ...
 */
declare function buildGraph(root: unknown, logger: Logger): {
  graph: Graph;
};
//#endregion
...
declare class Context<Spec extends Record<string, any> = any, Config extends AnyConfig = AnyConfig> {
  /**
...
declare namespace OpenApi {
  type V2_0_X = OpenAPIV2.Document;
  type V3_0_X = OpenAPIV3.Document;
  type V3_1_X = OpenAPIV3_1.Document;
}
declare namespace OpenApiMetaObject {
  type V2_0_X = OpenAPIV2.InfoObject;
  type V3_0_X = OpenAPIV3.InfoObject;
  type V3_1_X = OpenAPIV3_1.InfoObject;
}
declare namespace OpenApiOperationObject {
  type V2_0_X = OpenAPIV2.OperationObject;
  type V3_0_X = OpenAPIV3.OperationObject;
  type V3_1_X = OpenAPIV3_1.OperationObject;
}
declare namespace OpenApiParameterObject {
  type V3_0_X = OpenAPIV3.ParameterObject | OpenAPIV3.ReferenceObject;
  type V3_1_X = OpenAPIV3_1.ParameterObject | OpenAPIV3_1.ReferenceObject;
}
declare namespace OpenApiRequestBodyObject {
  type V3_0_X = OpenAPIV3.RequestBodyObject | OpenAPIV3.ReferenceObject;
  type V3_1_X = OpenAPIV3_1.RequestBodyObject | OpenAPIV3_1.ReferenceObject;
}
declare namespace OpenApiResponseObject {
  type V3_0_X = OpenAPIV3.ResponseObject | OpenAPIV3.ReferenceObject;
  type V3_1_X = OpenAPIV3_1.ResponseObject | OpenAPIV3_1.ReferenceObject;
}
declare namespace OpenApiSchemaObject {
  type V2_0_X = OpenAPIV2.SchemaObject;
  type V3_0_X = OpenAPIV3.SchemaObject;
  type V3_1_X = OpenAPIV3_1.SchemaObject;
}
//#endregion
...
//#endregion
//#region src/config/parser/types.d.ts
 ...
 */
interface BaseUserOutput<TModuleExtension extends string = string> {
  /**
 ...
 * Base output shape all packages must satisfy.
 */
interface BaseOutput<TModuleExtension extends string = string> {
  /**
   * Defines casing of the output fields. By default, we preserve `input`
   * values as data transforms incur a performance penalty at runtime.
   */
  ...
  /** Options for module specifier resolution. */
  module: {
    /** The extension used when importing other modules. */extension: TModuleExtension | AnyString | null; /** Function to transform module specifiers. */
 ...
 */
type BaseUserConfig<TOutput extends BaseUserOutput> = {
  /**
 ...
 * Core configuration shared across all packages.
 */
type BaseConfig<TUserConfig extends object, TOutput extends BaseOutput> = Omit<Required<TUserConfig>, 'input' | 'logs' | 'output' | 'parser' | 'plugins' | 'watch'> & {
  /**
 ...
 */
type AnyConfig = BaseConfig<Record<string, unknown>, BaseOutput>;
/**
 * Function to transform module specifiers.
 */
type ResolveModuleFn = (path: string, ctx: RenderContext) => string | undefined;
//#endregion
...
declare const debugTools: {
  graph: {
    readonly analyzeStructure: (graph: Graph) => {
      maxChildren: number;
      maxDepth: number;
      totalNodes: number;
    };
    readonly exportForVisualization: (graph: Graph) => {
      children: number;
  ...
  ir: {
    readonly print: (ir: IR.Model, options?: PrintOptions) => void;
  };
};
//#endregion
...
declare class HeyApiError extends Error {
...
declare function logCrashReport(error: unknown, logsDir: string): string | undefined;
declare function openGitHubIssueWithCrashReport(error: unknown, initialDir: string): Promise<void>;
declare function getInputError(error: unknown): InputError | undefined;
declare function printCrashReport({
  error,
  logPath
}: {
  error: unknown;
...
declare function shouldReportCrash({
  error,
  isInteractive
}: {
  error: unknown;
...
//#endregion
//#region src/fs.d.ts
declare function ensureDirSync(path: PathLike): void;
//#endregion
...
declare const hasOperationDataRequired: (operation: IR.OperationObject) => boolean;
declare const createOperationKey: ({
  method,
  path
}: {
  method: string;
...
declare const operationPagination: ({
  context,
  operation
}: {
  context: Context;
...
declare const statusCodeToGroup: ({
  statusCode
}: {
  statusCode: string;
}) => StatusGroup;
...
declare const operationResponsesMap: (operation: IR.OperationObject) => OperationResponsesMap;
//#endregion
//#region src/ir/parameter.d.ts
declare const hasParameterGroupObjectRequired: (parameterGroup?: Record<string, IR.ParameterObject>) => boolean;
declare const hasParametersObjectRequired: (parameters: IR.ParametersObject | undefined) => boolean;
declare const parameterWithPagination: ({
  context,
  parameters
}: {
  context: Context;
 ...
 */
declare function deduplicateSchema<T extends IR.SchemaObject>({
  detectFormat,
  schema
}: {
 ...
 */
declare function parseOpenApiSpec(context: Context): Context;
//#endregion
//#region src/openApi/2.0.x/parser/index.d.ts
declare const parseV2_0_X: (context: Context<OpenAPIV2.Document>) => void;
//#endregion
//#region src/openApi/3.0.x/parser/index.d.ts
declare const parseV3_0_X: (context: Context<OpenAPIV3.Document>) => void;
//#endregion
//#region src/openApi/3.1.x/parser/index.d.ts
declare const parseV3_1_X: (context: Context<OpenAPIV3_1.Document>) => void;
//#endregion
...
//#endregion
//#region src/plugins/duplicate.d.ts
type PluginConfig = {
  name: PluginNames;
};
 ...
 */
type ResolvedRequestValidatorLayer = Required<RequestValidatorLayer>;
/**
 ...
 */
type ValidatorLayers<Keys extends PropertyKey, Layer extends object> = { [K in Keys]?: Layer };
/**
 ...
 * @template Layer - The shape of an individual layer configuration.
 */
type DefaultValidatorLayers<Keys extends PropertyKey, Layer extends object> = { [K in Keys]: Required<Layer> };
/**
 * The ordered list of standard HTTP request layer keys.
 */
declare const requestValidatorLayers: readonly ["body", "headers", "path", "query"];
/**
 * Optional per-layer configuration for the standard HTTP request layers
 * (body, headers, path, query).
 */
type RequestValidatorLayers = ValidatorLayers<(typeof requestValidatorLayers)[number], RequestValidatorLayer>;
/**
 * Required default configuration for all standard HTTP request layers
 * (body, headers, path, query).
 */
type DefaultRequestValidatorLayers = DefaultValidatorLayers<(typeof requestValidatorLayers)[number], RequestValidatorLayer>;
/**
 * Context passed to request schema helpers.
 *
 * @template Plugin - The plugin instance type.
 */
interface RequestSchemaContext<Plugin extends PluginInstance = PluginInstance> {
  /**
 ...
 */
declare function resolveValidatorLayer<Layers extends Record<PropertyKey, object | undefined>, Key extends keyof Layers>(layers: Layers | undefined, key: Key, defaultValues: { [K in keyof Layers]-?: Required<NonNullable<Layers[K]>> }): Required<NonNullable<Layers[Key]>>;
//#endregion
...
//#endregion
//#region src/utils/naming/naming.d.ts
/**
 ...
 */
declare function toCase(value: string, casing: Casing | undefined, options?: {
  /**
 ...
 */
declare function resolveNaming(rule: NamingRule | undefined): NamingConfig;
/**
 * Apply naming configuration to a value.
 *
 * Casing is applied first, then transformation.
 */
declare function applyNaming(value: string, config: NamingConfig): string;
//#endregion
...
//#endregion
//#region src/utils/input/heyApi.d.ts
declare const heyApiRegistryBaseUrl = "https://get.heyapi.dev";
//#endregion
//#region src/utils/minHeap.d.ts
declare class MinHeap {
  declIndex: Map<string, number>;
  private heap;
  constructor(declIndex: Map<string, number>);
  isEmpty(): boolean;
 ...
 */
declare function refToName($ref: string): string;
/**
 ...
 */
declare function encodeJsonPointerSegment(segment: string | number): string;
/**
 ...
 */
declare function jsonPointerToPath(pointer: string): ReadonlyArray<string>;
/**
 ...
 */
declare function normalizeJsonPointer(pointer: string): string;
/**
