...
核心能力：Event 状态闭环 + 日历拖拽改期/拉伸时长 + 标签筛选 + 5 套主题 + 节日主题自动切换 + 特效系统（5 种）+ 键盘快捷键 + JWT 自动续签 + SSE 提醒推送 + ICS 导出 + PWA + 移动端适配 + 宠物养成（区域感知游走/小窝进窝休息/日程框互动：格内物理四边绕行+吸附落地弹跳/情绪/粒子；动作动画层：eat 进食 + idle 小动作 stretch/yawn/scratch/look + 情绪眨眼过渡；昼夜节律：夜间回窝/早晨唤醒问候/午后小憩/深夜打哈欠提示；健壮性：兴趣区惰性过期 + 游走节奏与渲染解耦）+ 任务看板（三列看板+列表+拖拽）
测试覆盖：37 类 259 用例（后端 H2，`<!-- DOCS-CHECK: backend-test-classes=37 -->`）+ 48 文件 232 用例（前端 vitest，`<!-- DOCS-CHECK: frontend-test-files=48 -->`）+ 11 文件 39 用例（Playwright E2E，`<!-- DOCS-CHECK: e2e-files=11 -->`）
