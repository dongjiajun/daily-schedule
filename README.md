...
CI 四道门禁：openspec-validation（OpenSpec 一致性：validate --all --strict + doctor + 主 spec 无 delta 头 + CLI 版本=CLAUDE.md 声明 + 归档完整性 validate --archived + test-plan 内容门禁）→ 版本一致性检查 → 后端 `mvn test` → 前端 `pnpm run lint` → `pnpm run test` → `pnpm run build`（含 SDK freshness check）；E2E（continue-on-error 软性，不阻断）
