 ...
 */
export function cellEdges(rect: { left: number; top: number; right: number; bottom: number }, bottomOnly = false): CellClingPoint[] {
  const width = rect.right - rect.left
  const height = rect.bottom - rect.top
  const marginX = width * EDGE_MARGIN_RATIO
  const marginY = height * EDGE_MARGIN_RATIO

  const points: CellClingPoint[] = []
  ...
  const cornerRB = { x: rect.right - marginX, y: rect.bottom - marginY, edge: 'right' as const }
  const cornerRT = { x: rect.right - marginX, y: rect.top + marginY, edge: 'right' as const }
  const cornerLT = { x: rect.left + marginX, y: rect.top + marginY, edge: 'left' as const }
  const cornerLB = { x: rect.left + marginX, y: rect.bottom - marginY, edge: 'bottom' as const }
 ...
 */
export function nextClingPoint(
  current: Position,
...
export function snapToEdge(
  pos: Position,
  edges: CellClingPoint[],
  threshold = CLING_SNAP_DISTANCE
): { pos: Position; snapped: boolean } {
  let nearest: CellClingPoint | null = null
  let nearestDist = Infinity
 ...
 */
export function landSnap(pos: Position, edges: CellClingPoint[]): Position {
  let nearest: CellClingPoint | null = null
  let nearestDist = Infinity
 ...
 * 配合到位吸附产生滑入→落定的过程感。
 */
export function slideInSpeed(baseSpeed: number, dist: number): number {
 ...
 */
export function hopOffset(t: number, height = HOP_HEIGHT): number {
 ...
 */
export function createCellStyle(completion: number): CellStyle {
  if (completion >= 50) {
    return {
      walkSpeed: 60,
      hopChance: 0.4,
 ...
 */
export function cellLapTarget(edgesLength: number, rounds = 2): number {
